...
const defaults = {
  ...
  intro: "A test game mode with custom weekly matchups.",
  leagueId: "",
  currentWeek: 1,
  heroImageUrl: "/photos/brass-balls/main-2026.png",
...
async function api(method, season, body) {
  const response = await fetch(
    `/api/admin/brass-balls?season=${encodeURIComponent(season)}`,
    {
      method,
      headers: {
...
export default function BrassBallsAdminClient() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setDoc({ ...defaults, season });
      setMessage(
        error.message.includes("not found")
          ? "No saved season yet. Start with the form below."
          : error.message,
      );
    } finally {
      setBusy(false);
  ...
  const updateWeek = (index, patch) =>
    setDoc((current) => ({
      ...current,
      weeks: current.weeks.map((row, i) =>
        i === index ? { ...row, ...patch } : row,
      ),
    }));
  const updateMatchup = (weekIndex, matchupIndex, side, rosterId) =>
    setDoc((current) => ({
      ...current,
  ...
  const save = async () => {
    ...
    setMessage("");
    try {
      await api("PUT", season, doc);
      setMessage("The Brass Balls schedule was published to R2.");
    } catch (error) {
  ...
  const chooseImage = async (file, field, section) => {
    if (!file) return;
    setBusy(true);
    setMessage("");
    try {
      const url = await uploadImage(file, section, season);
      setDoc((current) => ({ ...current, [field]: url }));
      setMessage("Image uploaded. Publish the page to save its placement.");
    } catch (error) {
      setMessage(error.message);
    } finally {
      setBusy(false);
  ...
  return (
    <section className="section">
      <div className="container-site max-w-6xl space-y-7">
        <AdminNav
          eyebrow="Admin · Game Modes"
          title="The Brass Balls"
              ...
              >
                <div className="flex flex-wrap items-end gap-3">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setDoc({
                        ...doc,
                        weeks: doc.weeks.filter((_, i) => i !== wi),
                      })
                    }
                ...
                <div className="mt-4 space-y-3">
                  {week.matchups.map((pair, mi) => (
                    ...
                    >
                      ...
                      >
                        ...
                        {teams.map((team) => (
                          <option
                            key={team.rosterId}
                            value={team.rosterId}
                            disabled={team.rosterId === pair.teamB?.rosterId}
                          >
                            {teamLabel(team)}
                          </option>
                        ))}
                      ...
                      >
                        ...
                        {teams.map((team) => (
                          <option
                            key={team.rosterId}
                            value={team.rosterId}
                            disabled={team.rosterId === pair.teamA?.rosterId}
                          >
                            {teamLabel(team)}
                          </option>
                        ))}
                      ...
                      <button
                        ...
                        onClick={() =>
                          updateWeek(wi, {
                            matchups: week.matchups.filter((_, i) => i !== mi),
                          })
                        }
                        className="btn btn-secondary"
                      >
                  ...
                  <button
                    ...
                    onClick={() =>
                      updateWeek(wi, {
                        matchups: [...week.matchups, emptyMatchup()],
                      })
                    }
                    className="btn btn-secondary"
                  >
            ...
            onClick={() =>
              setDoc({
                ...
                weeks: [
                  ...doc.weeks,
                  emptyWeek(
                    Math.max(0, ...doc.weeks.map((row) => num(row.week))) + 1,
                  ),
                ],
              })
            }
        ...
        {message ? (
          <div className="rounded-xl border border-subtle bg-card-surface p-4 text-sm">
            {message}
          </div>
        ) : null}
      </div>
