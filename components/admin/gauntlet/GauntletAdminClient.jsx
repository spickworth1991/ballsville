...
export default function GauntletAdminClient({ defaultSeason = DEFAULT_SEASON }) {
  ...
  async function load() {
    ...
    try {
      ...
      // Expand all by default if nothing set.
      setOpenLegions((prev) => {
        if (prev && prev.size) return prev;
        const next = new Set();
        for (const h of normalized.filter((x) => x.is_legion_header)) next.add(h.legion_slug);
        return next;
      });
    } catch (e) {
      setRows([]);
                                ...
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center justify-between gap-2">
                                    ...
                                    <button
                                      type="button"
                                      className="btn btn-outline btn-sm"
                                      onClick={() => openUpload({ kind: "league", legionSlug, leagueOrder: l.league_order, leagueKey: l.__key })}
                                    >
                                      Upload
                                    </button>
                                  </div>
