...
export default function RedraftAdminClient() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      const seeded = list.length ? list : [emptyLeague(1)];
      setLeagues(normalizeOrders(sortByOrder(seeded)));
    } catch (e) {
  ...
  function sortByOrder(arr) {
    ...
    return [...a].sort((x, y) => Number(x.order) - Number(y.order));
  }

  function normalizeOrders(next) {
    // IMPORTANT: do NOT sort here.
    // This only re-numbers in the *current array order*.
    // (Sorting inside normalizeOrders would undo "move up/down" swaps.)
    const a = Array.isArray(next) ? next : [];
    return a.map((l, idx) => ({ ...l, order: idx + 1 }));
  }
  ...
  function removeLeague(order) {
    setLeagues((prev) => normalizeOrders(sortByOrder(prev).filter((l) => Number(l.order) !== Number(order))));
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 text-white">
      ...
      <div className="grid gap-4">
        {sortedLeagues.map((l, idx) => {
          ...
          const sleeperUrl = l.sleeperUrl || (l.leagueId ? `https://sleeper.com/leagues/${l.leagueId}` : "");
