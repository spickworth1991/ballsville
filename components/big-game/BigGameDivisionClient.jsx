...
import { adminR2Url, adminR2UrlForKey } from "@/lib/r2Client";
...
function resolveImageSrc({ key, url }) {
  ...
  if (k) return k.startsWith("/r2/") ? adminR2Url(k) : adminR2UrlForKey(k);

  const u = safeStr(url).trim();
  return u ? adminR2Url(u) : "";
}
...
export default function BigGameDivisionClient({year = DEFAULT_SEASON,
  ...
  useEffect(() => {
    async function load() {
      ...
      setLoading(true);
      try {
        const res = await fetch(`${adminR2UrlForKey(R2_KEY_FOR(year))}?${bust}`, { cache: "no-store" });
