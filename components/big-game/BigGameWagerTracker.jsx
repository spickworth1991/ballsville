...
function ChampionshipCard({ champ }) {
  const entrants = safeArray(champ?.entrants);
  const pointsByOwner = champ?.pointsByOwner && typeof champ.pointsByOwner === "object" ? champ.pointsByOwner : {};
  const wagersByOwner = champ?.wagersByOwner && typeof champ.wagersByOwner === "object" ? champ.wagersByOwner : {};
  ...
  const rows = entrants
    .map((e) => {
      ...
      return {
        ...
        wager: Number(wagersByOwner[ownerName] || 0),
        points: Number(pointsByOwner[ownerName] || 0),
      };
    })
    .sort((a, b) => b.wager - a.wager || b.points - a.points || a.ownerName.localeCompare(b.ownerName));

  const pool = rows.reduce((sum, r) => sum + (Number.isFinite(r.wager) ? r.wager : 0), 0);
  const winner = safeStr(champ?.winner);
  ...
  return (
    <div className="card bg-card-surface border border-subtle rounded-2xl shadow-md overflow-hidden">
      <div className="p-5 border-b border-subtle flex items-center justify-between gap-3">
        ...
        <div className="text-xs text-muted">
          <span className="inline-flex items-center gap-2 rounded-full border border-subtle bg-panel/60 px-3 py-1">
            Pool: <span className="text-foreground font-semibold">{fmtMoney(pool)}</span>
          </span>
        </div>
      </div>
      ...
      <div className="p-5">
        {winner ? (
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-emerald-300/30 bg-emerald-500/10 px-3 py-1">
            <span className="text-emerald-200 text-[11px] uppercase tracking-[0.2em]">Champion</span>
