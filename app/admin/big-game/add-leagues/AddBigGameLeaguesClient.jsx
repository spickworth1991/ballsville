...
export default function AddBigGameLeaguesClient({ initialSeason }) {
  ...
  const [bigGameLeaguesData, setBigGameLeaguesData] = useState(null);
  ...
  const rows = useMemo(() => {
    const r = bigGameLeaguesData?.rows;
  ...
  const divisions = useMemo(() => {
    ...
    return headers
      .map((h) => ({
        division_name: safeStr(h?.division_name || "Division"),
        division_slug: safeStr(h?.division_slug || "").trim(),
        division_order: safeNum(h?.division_order, 0),
      }))
  ...
  useEffect(() => {
    (async () => {
      try {
        if (!seasonNum) return;
        const data = await fetchJson(`/api/admin/biggame?season=${encodeURIComponent(String(seasonNum))}&type=leagues`);
        setBigGameLeaguesData(data?.data || { rows: [] });
      } catch (e) {
        setBigGameLeaguesData({ rows: [] });
      }
    })();
  }, [seasonNum]);
  ...
  async function addSelectedToBigGame() {
    ...
    try {
      ...
      const existingSleeperIds = new Set(
        rows
          .map((r) => safeStr(r?.sleeper_league_id || "").trim())
          .filter(Boolean)
      );

      const targetHeader = rows.find(
        (r) => r?.is_division_header && safeStr(r?.division_slug || "").trim() === targetDivision
      );
      if (!targetHeader) throw new Error("Target division header not found.");

      const division_name = safeStr(targetHeader?.division_name || "Division");
      const division_slug = safeStr(targetHeader?.division_slug || targetDivision);
      const division_order = safeNum(targetHeader?.division_order, 0);

      // next display_order within division
      const existingInDiv = rows.filter(
        (r) => !r?.is_division_header && safeStr(r?.division_slug || "").trim() === targetDivision
      );
      ...
      const toAdd = sleeperLeagues
        ...
        .filter((lg) => !existingSleeperIds.has(String(lg.league_id)))
        .map((lg) => {
          order += 1;
          ...
          return {
            ...
            division_slug,
            division_order,
            division_status: safeStr(targetHeader?.division_status || "TBD"),
            division_blurb: safeStr(targetHeader?.division_blurb || ""),
            division_image_key: safeStr(targetHeader?.division_image_key || ""),
            division_image_path: safeStr(targetHeader?.division_image_path || ""),
      ...
      if (!toAdd.length) throw new Error("No new leagues to add (selected leagues are already present).");

      const payload = {
        rows: [...rows, ...toAdd],
      };

      await fetchJson(`/api/admin/biggame?season=${encodeURIComponent(String(seasonNum))}&type=leagues`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });

      const updated = await fetchJson(`/api/admin/biggame?season=${encodeURIComponent(String(seasonNum))}&type=leagues`);
      setBigGameLeaguesData(updated?.data || { rows: [] });
      setSelectedIds(new Set());
      setInfoMsg(`Added ${toAdd.length} leagues to ${division_name}. Set invite links back on the Big Game admin page.`);
    } catch (e) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="space-y-1">
          <h1 className="text-xl font-semibold">Big Game – Add Leagues from Sleeper</h1>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <div className="card p-4 space-y-3">
          <div className="space-y-1">
            ...
            <div className="text-xs text-muted">Divisions come from the Big Game leagues JSON for that season.</div>
          </div>
        ...
        <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
          <div>
            ...
            <div className="text-xs text-muted">
              Counts are computed from Sleeper metadata when available. Big Game invite links are still manual, so you’ll set them on the main admin page.
            </div>
          </div>
