...
export default function DraftCompareBuildHomePage() {
  ...
  const canProceed = useMemo(() => {
    return !!modeSlug && !!title && Number.isFinite(year) && year > 2000 && Number.isFinite(order) && Number.isFinite(season);
        ...
        <div className="rounded-3xl border border-border bg-card-surface/80 p-6 backdrop-blur">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-2 text-2xl font-extrabold tracking-tight text-primary">{action === "rebuild" ? "Rebuild" : "Create"} Draft Database</h1>
              <p className="mt-2 text-sm text-muted">
                Mode: <span className="font-semibold text-primary">{title || "—"}</span> • Sleeper season: <span className="font-semibold text-primary">{year || "—"}</span>
              </p>
              <p className="mt-1 text-xs text-muted">This will generate the exact same Draft Compare JSON, then save it to R2 for this mode.</p>
            </div>
            ...
            <form onSubmit={goNext} className="rounded-2xl border border-border bg-background/30 p-4">
              ...
              <button
                ...
                title={
                  canProceed
                    ? ""
                    : "Go back and fill in Order, Year, and Title for this mode first."
                }
              >
