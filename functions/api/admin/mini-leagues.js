...
async function touchManifest(env, season) {
  ...
  if (!b.ok) return;
  const key = season ? `data/manifests/mini-leagues_${season}.json` : `data/manifests/mini-leagues.json`;
  const body = JSON.stringify({ section: "mini-leagues", season: season || null, updatedAt: new Date().toISOString() }, null, 2);
  await b.bucket.put(key, body, { httpMetadata: { contentType: "application/json; charset=utf-8", cacheControl: "no-store" } });
}
