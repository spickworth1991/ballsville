...
export function normalizeDraftCompareJson(input) {
  ...
  return { meta: { teams, rounds, draftType: safeStr(meta?.draftType), isAuction: meta?.isAuction === true }, leagues };
}
...
export function aggregateSelectedLeagues(norm, selectedLeagueIds) {
  ...
  function addLeaguePick({ name, position, round, pickInRound, overallPick, winningBid }) {
    ...
    if (Number.isFinite(bid) && bid >= 0) {
      playersAgg[key].sumBid += bid;
      playersAgg[key].bidCount += 1;
    }
  ...
  for (const lg of leagues) {
    ...
    if (picks.length) {
      for (const pk of picks) {
        ...
        addLeaguePick({ name, position, round, pickInRound, overallPick, winningBid: po?.winningBid ?? po?.amount });
      }
      ...
      for (const k0 of playerKeys) {
        ...
        if (parsed.round && parsed.pick) {
          ...
          addLeaguePick({
            ...
            winningBid: p?.avgWinningBid ?? p?.winningBid ?? p?.amount,
          });
          continue;
        }
        ...
        if (o > 0) {
          ...
          if (p2.round && p2.pick) {
            addLeaguePick({ name, position, round: p2.round, pickInRound: p2.pick, overallPick: o, winningBid: p?.avgWinningBid ?? p?.winningBid ?? p?.amount });
          }
        }
      }
  ...
  for (const key of Object.keys(playersAgg)) {
    ...
    playersOut[key] = {
      ...
      ...(p.bidCount > 0 ? { avgWinningBid: p.sumBid / p.bidCount } : {}),
    };
  }
  ...
  return {
    meta: {
      teams: safeNum(meta?.teams) || 12,
      rounds: safeNum(meta?.rounds) || 18,
      draftType: safeStr(meta?.draftType),
      isAuction: meta?.isAuction === true || (leagues.length > 0 && leagues.every((l) => l?.meta?.isAuction === true || safeStr(l?.meta?.draftType).toLowerCase() === "auction")),
    },
    leagueCount,
    players: playersOut,
...
export function buildPlayerResults(groupA, groupB) {
  ...
  for (const v of map.values()) {
    ...
    out.push({
      ...
      roundPickA,
      roundPickB,
      winningBidA: v.a && Number.isFinite(Number(v.a.avgWinningBid)) ? Number(v.a.avgWinningBid) : null,
      winningBidB: v.b && Number.isFinite(Number(v.b.avgWinningBid)) ? Number(v.b.avgWinningBid) : null,
    });
  }
