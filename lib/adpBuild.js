...
import { getDraftPicks, getLeagueDraftById, getLeaguePrimaryDraft } from "./sleeperApi.js";
...
function finalizePlayersMap(map, teams) {
  ...
  for (const [k, acc] of map.entries()) {
    ...
    out[k] = {
      ...
      ...(acc.bidCount > 0 ? { avgWinningBid: acc.sumBid / acc.bidCount } : {}),
    };
  }
  return out;
}
    ...
    out[cellKey] = Array.from(pmap.values())
      .map((v) => {
        ...
        return {
          ...
          ...(v.bidCount > 0 ? { avgWinningBid: v.sumBid / v.bidCount } : {}),
        };
      })
      .sort((a, b) => {
...
export async function getADPGroupData(leagueDraftKeys) {
  ...
  const draftTypes = [...new Set(valid.map((d) => safeStr(d?.type).trim().toLowerCase()).filter(Boolean))];
  const isAuction = draftTypes.length > 0 && draftTypes.every((type) => type === "auction");
  const meta = { teams: firstTeams, rounds: firstRounds, draftType: isAuction ? "auction" : draftTypes[0] || "", isAuction };
  ...
  for (let i = 0; i < pickLists.length; i++) {
    ...
    for (const p of picks) {
      ...
      const amount = safeNum(p?.metadata?.amount ?? p?.amount);
      ...
      if (!acc) {
        acc = { name, pos, sumPick: 0, count: 0, pickCounts: new Map(), sumBid: 0, bidCount: 0 };
        byPlayer.set(key, acc);
      }
      acc.sumPick += pickNo;
      acc.count += 1;
      acc.pickCounts.set(pickNo, (acc.pickCounts.get(pickNo) || 0) + 1);
      if (amount != null) { acc.sumBid += amount; acc.bidCount += 1; }

      // Per-league player acc
      let acc1 = byPlayerOne.get(key);
      if (!acc1) {
        acc1 = { name, pos, sumPick: 0, count: 0, pickCounts: new Map(), sumBid: 0, bidCount: 0 };
        byPlayerOne.set(key, acc1);
      }
      acc1.sumPick += pickNo;
      acc1.count += 1;
      acc1.pickCounts.set(pickNo, (acc1.pickCounts.get(pickNo) || 0) + 1);
      if (amount != null) { acc1.sumBid += amount; acc1.bidCount += 1; }
      ...
      if (!cc) {
        cc = { name, pos, sumPick: 0, count: 0, sumBid: 0, bidCount: 0 };
        cellMap.set(key, cc);
      }
      cc.sumPick += pickNo;
      cc.count += 1;
      if (amount != null) { cc.sumBid += amount; cc.bidCount += 1; }
      ...
      if (!cc1) {
        cc1 = { name, pos, sumPick: 0, count: 0, sumBid: 0, bidCount: 0 };
        cellMap1.set(key, cc1);
      }
      cc1.sumPick += pickNo;
      cc1.count += 1;
      if (amount != null) { cc1.sumBid += amount; cc1.bidCount += 1; }
    }
    ...
    leagues.push({
      ...
      meta: { ...meta, draftType: safeStr(draftMeta?.type).trim().toLowerCase(), isAuction: safeStr(draftMeta?.type).trim().toLowerCase() === "auction" },
